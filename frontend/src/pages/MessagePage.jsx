...
const MessagePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchGroupData = async () => {
      try {
        ...
        setGroupName(response.data.name); // Assuming the response contains a group name
      } catch (err) {
  ...
  const sendMessage = () => {
    if (newMessage.trim()) {
      const messageData = { groupId, message: newMessage, senderName: userName };
  ...
  return (
    <div className="bg-light dark:bg-gray-900 min-h-screen p-8 transition-colors duration-300">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="messages-container h-96 overflow-y-scroll bg-gray-100 dark:bg-gray-700 p-4 rounded-lg flex flex-col mb-4 shadow-md">
          {messages.map((msg, index) => (
            <div
              key={index}
              className={`message mb-3 p-3 rounded-lg ${msg.senderName === userName ? 'bg-blue-500 text-white self-end' : 'bg-gray-200 dark:bg-gray-600 text-gray-800 self-start'}`}
              style={{ maxWidth: '75%' }}
            >
              <p className="font-semibold text-sm text-gray-900 dark:text-white">{msg.senderName}</p>
        ...
        <div className="message-input flex items-center">
          <input
            ...
            className="flex-grow p-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <button
            onClick={sendMessage}
